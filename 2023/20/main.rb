...
def reset_modules(input)
  ...
  modules.clone.each do |n,m|
    m[:targets].each do |t|
      ...
      (modules[t][:mem] ||= {})[n] = :low if modules[t][:type] == :&
    end
  end
...
def push(modules, part2_mod = nil)
  ...
  until pulses.empty?
    ...
    count[pulse] += 1
    count[:p2_high_inp] = src if trgt == part2_mod && pulse == :high
    ...
    when :%
      next if pulse == :high
      nxt_pulse = (mod[:on] = !mod[:on]) ? :high : :low
    when :&
      mod[:mem][src] = pulse
      nxt_pulse = mod[:mem].all?{ _2 == :high } ? :low : :high
    end
...
start = Time.now
modules = reset_modules(input)
part1 = 1000.times.inject([0,0]){ |s,_| c = push(modules); [s[0]+c[:low],s[1]+c[:high]] }.inject(&:*)
puts "Part 1: #{part1} (#{Time.now - start}s)"


start = Time.now
modules = reset_modules(input)
part2_mod = modules.keys.detect{ modules[_1][:targets].include?(:rx) }
high_inp = {}
1.step do |i|
